defmodule ReportsGenerator do
  ...
  def fetch_higher_cost(_report, _option),
    do: {:error, "Invalid option: The argument 'option' accept only 'users' OR 'foods'."}

  def fetch_lower_cost(report, option) when option in @options do
    {:ok, Enum.min_by(report[option], fn {_key, value} -> value end)}
  end

  def fetch_lower_cost(_report, _option),
    do: {:error, "Invalid option: The argument 'option' accept only 'users' OR 'foods'."}
